---
title: 界面协议
description: 自定义任务面板、追踪 HUD、附近 NPC HUD 与对话框时使用的动作和数据结构
---
...
追踪 HUD 与附近 NPC HUD 都没有客户端动作，服务端会在内容或选中项变化时主动同步。附近
...
| 附近 NPC HUD | `interaction_sync`   | `InteractionSnapshot`  | 候选列表发生变化时的完整快照 |
| 附近 NPC HUD | `interaction_select` | `InteractionSelection` | 候选初始化或快捷栏槽位变化时 |
...
服务端通过 `interaction_sync` 发送完整候选快照：

```json
{
  "entries": [
    {
      "id": "2ef7b9b0-2c07-4c94-b4c7-a637ef5a1e89",
      "name": "§b边境档案员"
    },
    {
      "id": "501ec7e1-f320-4d75-a0f3-fc5310c7b61b",
      "name": "§b巡林联络员"
    }
  ]
}
...
| 字段               | 类型                        | 说明                                 |
|------------------|---------------------------|------------------------------------|
| `entries`        | `Array<InteractionEntry>` | 当前可交互 NPC 的完整有序列表                  |
| `entries[].id`   | String                    | Adyeshach 实体 UUID，用于动态控件稳定命名和匹配选中项 |
| `entries[].name` | String                    | NPC 显示名                            |


服务端通过 `interaction_select` 发送当前选中项：

```json
{
  "selected": "19f43df7-5388-430f-bd3d-1604488bf624"
}
```
