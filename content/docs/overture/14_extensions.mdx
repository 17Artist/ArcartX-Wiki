...
|---------------------------------|-------------------------------------|------------------------------------------------|-----------------------------------|
| 根据玩家、当前物品生成名称、Lore 或渲染 NBT      | RenderEntry                         | `player`、只读物品 clone、`displayId`、`ItemDataView` | 否                                 |
...
lore:
  - "<example:item_desc...>"
nbt:
  tooltip_title: "<example:item_desc>"
  tooltip_lines:
    - "<example:item_desc...>"
```

`RenderEntryRenderer` 接收 `RenderEntryContext`，其中包含 `key`、`player`、`itemStack`、`displayId` 与只读 `data`。名称和 `nbt` 字符串使用第一行，Lore 与 `nbt` 列表可展开全部行。同一标签每次渲染只调用一次，即使同一个标签同时出现在这几个位置。回调仅在主线程执行；`itemStack` 与 `data` 都来自 clone，修改不会写回真实物品。
