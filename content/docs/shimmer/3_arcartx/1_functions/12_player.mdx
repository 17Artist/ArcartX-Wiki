...
### `Player.getOnlinePlayersDict(){:Shimmer}`
- 获取在线玩家列表
<Callout type="info">
- **参数**：无
...
## 状态信息
### `Player.getHealth(){:Shimmer}` / `Player.getMaxHealth(){:Shimmer}`
- 获取玩家当前/最大生命值
<Callout type="info">
- **参数**：无
- **返回值**：浮点数类型
...
### `Player.getLevel(){:Shimmer}` / `Player.getExp(){:Shimmer}`
- 获取玩家等级/经验值
<Callout type="info">
- **参数**：无
- **返回值**：整数/浮点数类型
...
## 位置信息
### `Player.getPosX(){:Shimmer}` / `Player.getPosY(){:Shimmer}` / `Player.getPosZ(){:Shimmer}`
- 获取玩家X/Y/Z坐标
<Callout type="info">
- **参数**：无
- **返回值**：双精度浮点数类型
...
### `Player.getYaw(){:Shimmer}` / `Player.getPitch(){:Shimmer}`
- 获取玩家偏航角/俯仰角
<Callout type="info">
- **参数**：无
- **返回值**：浮点数类型
...
## 状态检测
### `Player.isFlying(){:Shimmer}` / `Player.isInWater(){:Shimmer}` / `Player.isOnGround(){:Shimmer}`
- 检测玩家飞行/在水中/在地面状态
<Callout type="info">
- **参数**：无
- **返回值**：布尔值类型
...
## 其他属性
### `Player.getFood(){:Shimmer}` / `Player.getAir(){:Shimmer}` / `Player.getArmor(){:Shimmer}`
- 获取玩家饥饿值/氧气值/护甲值
<Callout type="info">
- **参数**：无
- **返回值**：整数类型
...
### `Player.getMainHandItem(){:Shimmer}`
- 获取玩家主手物品
<Callout type="info">
    - **参数**：无
    - **返回值**：ItemStack对象
