...
## `Keyboard.isKeyDown(key){:Shimmer}`
- 检测指定按键是否被按下
...
## `Keyboard.getKeyBindingKey(binding){:Shimmer}`
...
## `Keyboard.getKeyBindingKeyName(binding){:Shimmer}`
...
## `Keyboard.getKeyBindingKeyModifierName(binding){:Shimmer}`
- 获取按键绑定的修饰键名称
...
## `Keyboard.setKeyBindingKeyName(binding, modifier, keyName){:Shimmer}`
- 设置按键绑定的按键
...
## `Keyboard.keyPress(key, pressed){:Shimmer}`
- 模拟按键按下或释放
