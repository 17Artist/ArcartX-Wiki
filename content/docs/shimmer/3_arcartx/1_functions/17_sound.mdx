...
## 音效播放
### `Sound.local(identifier, x, y, z, category, distance, pitch, duration){:Shimmer}`
- 在指定位置播放3D音效
...
### `Sound.entity(identifier, target, category, distance, pitch, duration){:Shimmer}`
- 为指定实体播放3D音效
...
### `Sound.named(dentifier, pitch, duration,name){:Shimmer}`
...
### `Sound.removeNamed(name){:Shimmer}`
- 停止播放命名音效
<Callout type="info">
    - **参数**：
    - `name`: 字符串类型(音效名称)
