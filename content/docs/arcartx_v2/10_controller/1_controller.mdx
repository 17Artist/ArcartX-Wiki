...
- 保留名：如 `idle`、`walk`、`run`、`swim` 等，控制器会自动判断玩家行为来触发。完整列表见[状态保留名](/docs/arcartx_v1/core/10_controller/2_state_name)
...
| onStart    | string  |  ""   | 进入状态时执行的 Aria 脚本                       |
| onEnd      | string  |  ""   | 离开状态时执行的 Aria 脚本                       |
