...
# 示例4. 通过ArcartXAPI让玩家客户端执行Aria脚本，配置中调用方式：执行脚本{script = Aria.println("Hello World!")}
执行脚本:
  type: "normal"
  script: |
    ArcartXAPI.static.getEntityManager().getPlayer(player).parseAria(args.getString("script"));
