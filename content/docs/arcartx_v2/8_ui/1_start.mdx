...
## 前言
- 学习 UI 之前，请先学习 Aria 的使用，UI 的中后期应用是围绕脚本展开的。
- 如果你只需要制作简单的 UI，且运算仅涉及基础的数字加减乘除，可以跳过 Aria 部分。
- UI 的讲解中会穿插部分开发相关内容，请根据需要自行参考。
<Callout>
    - 以下是一条重要提示，请务必留意。
    - UI 几乎所有属性都是脚本语句，因此如果某个属性的值是字符串，无法直接作为文本读取。
    - 因为 Aria 的字符串使用单引号包裹，比如`"abc"{:aria}`，而 YAML 语法中引号涉及转义问题。
...
## UI的上下文逻辑
- 在学习完 Aria 之后，下面简单讲解一下 UI 的上下文逻辑。
...
- 在Aria教学中我们提到过载体关键字:`self`，这里讲解一下不同位置的`self`所指向的对象。
...
- 对于`template`块，写法和`controls`一模一样，但是这个块中的控件是作为模板存在，不会被创建，需要主动使用create函数来加入到UI中。用于动态创建UI的需求（后续请看UI的Aria函数和控件的Aria函数）