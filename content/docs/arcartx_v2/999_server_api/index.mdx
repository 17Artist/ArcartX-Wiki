...
- ArcartX的所有API都通过 `ArcartXAPI` 这个入口类获取，它提供了9个静态方法，分别返回各个功能模块的管理器单例。
...
ArcartXAPI.getChatCardRegistry();        // 聊天卡片注册
...
ArcartXAPI.getInteractionProxyManager(); // 交互代理（命中盒代理）管理
```
