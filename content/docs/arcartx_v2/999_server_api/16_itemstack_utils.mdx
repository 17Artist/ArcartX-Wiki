...
### `ItemStack.hasLore(text){:java}`
- 判断物品Lore中是否包含与指定文本完全相等的行（精确匹配）

<Callout type="info">
- **参数**：`text`: String — 要匹配的完整行文本
- **返回值**：boolean
- **调用示例**
```java
boolean has = ItemStackUtils.hasLore(itemStack, "§a这是一行绿色Lore");
```
</Callout>
...
### `ItemStack.setArmorTexture(id){:java}`
- 设置物品作为盔甲穿戴时的盔甲贴图ID
...
- **调用示例**
```java
ItemStackUtils.setArmorTexture(itemStack, "knight_armor");
```
</Callout>

### `ItemStack.setCostume(modelId, hide){:java}`
- 设置物品的时装模型ID，并控制是否隐藏物品本体

<Callout type="info">
- **参数**：
    - `modelId`: String — 时装模型ID
    - `hide`: boolean — 是否隐藏物品本体（`true` 隐藏，`false` 显示）
- **返回值**：无
- **调用示例**
```java
ItemStackUtils.setCostume(itemStack, "wings_costume", true);
```
</Callout>
