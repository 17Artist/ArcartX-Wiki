...
## 模型格式
- 关于ArcartX的模型格式，是ArcartX的特化格式，我们支持通用模型的大部分能力，比如网格、多贴图以及基础的Cube几何能力。
- 此外，我们支持序列帧、相机动画、显示调整等原通用模型未提供的能力。
- 该格式可使用我们的插件来注册到你的BlockBench：[点我下载](https://arcartx.com/resources/140/)

## 模型渲染
- 对于模型渲染，我们提供了丰富的挂载格外特效，让你做出更丰富的视觉效果。此外，我们提供多层优化的CPU渲染路径和GPU渲染路径。
- 在未开启光影（仅对Iris系光影提供支持，OPF无法使用）的时候，将使用高性能GPU渲染路径。
- 在开启光影的时候，由于其覆盖了Shader，走多层CPU优化路径（毋庸置疑，性能会低于GPU渲染，但已尽可能剔除视觉不可见的顶点）。
