...
- 收件箱固定 ID：`SystemMail:inbox`；
- HUD 固定 ID：`SystemMail:hud`；
- 页码从 `0` 开始，`0` 是最新一页；
...
每页数量由 `mail.yml -> limits.inbox-page-size` 决定，默认 `20`，允许 `1–100`。

成功回包使用 `list`，内容为 `List<Map>`：
...
无结果时返回 `[]`。`含附件` 只匹配有可领取附件的邮件。

`systemmail_page_count` 是未筛选收件箱的总页数，最小为 `1`，不代表当前搜索结果的页数。当前回包少于每页数量时可以确定没有下一页；刚好等于每页数量时，应结合该变量或允许用户继续翻页。
...
- 文案来自服务端 `language.yml` 的 `failure.*`。
...
| `systemmail_page_count`   | 未筛选收件箱总页数；最小为 1，空收件箱保持在第 1 页 |

未读数会在玩家进入服务器、收到新邮件、读取邮件或附件状态变化后刷新；总页数会在打开收件箱后刷新。

## 新邮件 HUD

新邮件使用 handler `new_mail`：
